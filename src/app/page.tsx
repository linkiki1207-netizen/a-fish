...
import { 
  ...
  Package, ShieldCheck, Sparkles, Clock, CreditCard, Send, MapPin, ExternalLink, Archive, Boxes, Globe, User, Phone, Save
} from 'lucide-react'
...
function StoreContent() {
  ...
  const handleReportPayment = async (e: React.FormEvent) => {
    ...
    if (!bankLast5.trim() || !shippingName.trim() || !shippingPhone.trim() || !shippingStore.trim()) {
      alert('請完整填寫匯款後五碼與透過地圖選取 7-11 寄貨門市！')
      return
    }
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-6">
        <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl max-w-sm w-full text-center shadow-2xl space-y-4">
          ...
          <h2 className="text-2xl font-bold text-white">🎉 喊單登記成功！</h2>
                  ...
                  <div>
                    {product.image_url ? (
                      <img
                        ...
                        alt={product.name}
                        className="w-full h-52 object-cover rounded-xl mb-3.5 bg-slate-800"
                      />
                    ) : (
                    ...
                    <h3 className="font-bold text-slate-100 text-lg pr-16 leading-snug">{product.name}</h3>
            ...
            {memberTab === 'profile' ? (
              <form onSubmit={handleSaveProfile} className="space-y-4 text-xs">
                ...
                <div className="space-y-3">
                  <div>
                    <label className="text-slate-300 block mb-1">真實姓名 (領件核對證件)</label>
                    <input
                      type="text"
                      required
                      placeholder="例：林小魚"
                  ...
                  <div>
                    <label className="text-slate-300 block mb-1">手機號碼 (接收到貨簡訊)</label>
                    <input
                      type="tel"
                      required
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1">
                      <label className="text-slate-300">常備 7-11 門市</label>
                      <button
                        type="button"
                        onClick={open711Map}
        ...
        <div className="fixed inset-0 bg-black/85 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl max-h-[92vh] overflow-y-auto">
            ...
            <form onSubmit={handleReportPayment} className="space-y-4 text-sm">
              <div>
                <label className="text-slate-300 block mb-1.5 font-medium text-xs">匯款完成後，請輸入帳號末五碼：</label>
                <input
                  type="text"
                  required
                  maxLength={5}
              ...
              <div className="pt-3 border-t border-slate-800 space-y-3">
                ...
                <div>
                  <label className="text-slate-300 block mb-1 text-xs">取件人真實姓名 (領件需核對證件)</label>
                  <input
                    type="text"
                    required
                    placeholder="例：林小魚"
                ...
                <div>
                  <label className="text-slate-300 block mb-1 text-xs">取件人手機號碼 (接收到貨簡訊)</label>
                  <input
                    type="tel"
                    required
                    placeholder="例：0912345678"
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <label className="text-slate-300 text-xs">7-11 門市名稱與店號</label>
                    <button
                      type="button"
                      onClick={open711Map}
      ...
      {addedItemModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-sm w-full p-6 text-center space-y-4 shadow-2xl">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-100">已加入清單！</h3>
      ...
      {isCartOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            {cart.length === 0 ? (
              <p className="text-slate-400 text-sm text-center py-10">目前沒有選取任何商品</p>
            ) : (
              <div className="space-y-4">
                ...
                <form onSubmit={handleSubmitOrder} className="space-y-3 pt-2 border-t border-slate-800">
                  ...
                  <button
                    ...
                    className="w-full mt-2 py-3.5 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-800 disabled:text-slate-500 text-white font-bold rounded-xl text-sm md:text-base transition shadow-lg shadow-emerald-900/40 cursor-pointer"
                  >
                    {submitting ? '登記中...' : '⚡️ 登記搶單 (可持續追加商品)'}
                  </button>
                </form>
              </div>
            )}
