...
import { 
  ...
  Copy, MessageSquare, CheckCircle, Truck, Clock, MapPin, Phone, User, RotateCcw, Archive, Printer, ClipboardCheck, AlertCircle
} from 'lucide-react'
...
export default function AdminOrdersPage() {
  ...
  const handleMarkAsShipped = async (orderIds: string[]) => {
    const { error } = await supabase
      .from('orders')
      .update({ status: 'shipped', pay_status: 'paid' })
      .in('id', orderIds)
  ...
  const handleMarkAsCompleted = async (orderIds: string[]) => {
    const { error } = await supabase
      .from('orders')
      .update({ status: 'completed', pay_status: 'paid' })
      .in('id', orderIds)
  ...
  return (
    <div className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-300 mt-1">
            出貨寄出後訂單將自動歸檔，讓當前作業畫面保持乾淨！
          </p>
        </div>

        <div className="flex items-center gap-3">
          <div className="bg-slate-900 border border-slate-800 px-5 py-2.5 rounded-2xl flex items-center gap-3">
            <span className="text-sm text-slate-300">
              {orderTab === 'completed' ? '已完成訂單總額' : '分頁應收總額'}
            </span>
            <span className="text-emerald-400 font-mono font-bold text-lg">
              NT$ {totalCalculatedRevenue.toLocaleString()}
            </span>
          </div>
          <button
            onClick={fetchData}
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-2 p-1.5 bg-slate-900 border border-slate-800 rounded-2xl max-w-4xl">
        <button
          onClick={() => { setOrderTab('active'); setSelectedBatch('ALL'); }}
          className={`py-2.5 rounded-xl text-xs font-bold transition flex items-center justify-center gap-1 cursor-pointer ${
            orderTab === 'active' ? 'bg-emerald-500 text-slate-950 shadow-md' : 'text-slate-300 hover:text-white'
          }`}
        >
        ...
        <button
          onClick={() => { setOrderTab('reported'); setSelectedBatch('ALL'); }}
          className={`py-2.5 rounded-xl text-xs font-bold transition flex items-center justify-center gap-1 cursor-pointer ${
            orderTab === 'reported' ? 'bg-amber-500 text-slate-950 shadow-md' : 'text-slate-300 hover:text-white'
          }`}
        >
        ...
        <button
          onClick={() => { setOrderTab('shipping'); setSelectedBatch('ALL'); }}
          className={`py-2.5 rounded-xl text-xs font-bold transition flex items-center justify-center gap-1 cursor-pointer ${
            orderTab === 'shipping' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-300 hover:text-white'
          }`}
        >
        ...
        <button
          onClick={() => { setOrderTab('shipped'); setSelectedBatch('ALL'); }}
          className={`py-2.5 rounded-xl text-xs font-bold transition flex items-center justify-center gap-1 cursor-pointer ${
            orderTab === 'shipped' ? 'bg-purple-600 text-white shadow-md' : 'text-slate-300 hover:text-white'
          }`}
        >
        ...
        <button
          onClick={() => { setOrderTab('completed'); setSelectedBatch('ALL'); }}
          className={`py-2.5 rounded-xl text-xs font-bold transition flex items-center justify-center gap-1 cursor-pointer ${
            orderTab === 'completed' ? 'bg-slate-800 text-slate-100 shadow-md border border-slate-700' : 'text-slate-300 hover:text-white'
          }`}
        >
      ...
      {loading ? (
        ...
        <div className="bg-slate-900/40 border border-dashed border-slate-800 rounded-3xl p-16 text-center text-slate-400 text-sm">
          目前此分頁沒有任何訂單
        </div>
      ) : (
            ...
            return (
              <div
                ...
                className={`bg-slate-900 border rounded-3xl p-6 space-y-4 shadow-xl transition ${
                  isCompleted 
                    ? 'border-slate-800/50 bg-slate-950/60 opacity-80' 
                    : isShipped
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 border-b border-slate-800 pb-5">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="space-y-1">
                      <div className="flex flex-wrap items-center gap-2.5">
                        ...
                        <span className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 px-3 py-1 rounded-lg text-xs font-bold">
                          {getBatchBadgeWithFlag(g.batch_name)}
                        </span>
                        
                        <span className="text-xs text-slate-300 font-mono">
                          🕒 {new Date(g.latest_time).toLocaleString('zh-TW', { hour12: false })}
                        </span>
                        ...
                        {isCompleted ? (
                          ...
                          <span className="text-xs bg-purple-500/20 text-purple-300 px-2.5 py-1 rounded-md font-bold flex items-center gap-1">
                            <Package className="w-3.5 h-3.5" /> 已寄出交貨便
                          </span>
                        ) : isPaid ? (
                          <span className="text-xs bg-blue-500/20 text-blue-300 px-2.5 py-1 rounded-md font-bold flex items-center gap-1">
                            <Truck className="w-3.5 h-3.5" /> 已入帳・準備出貨
                          </span>
                        ) : isReported ? (
                          <span className="text-xs bg-amber-500/20 text-amber-300 px-2.5 py-1 rounded-md font-bold animate-pulse flex items-center gap-1">
                            <Clock className="w-3.5 h-3.5" /> 已回報匯款與門市
                          </span>
                        ) : (
                          <span className="text-xs bg-slate-950 text-slate-300 px-2.5 py-1 rounded-md">
                            待處理 (未填資料)
                          </span>
                        )}
                      </div>
                      ...
                      {g.store_name && (
                        <div className="mt-2 p-2.5 bg-slate-950 rounded-xl border border-slate-800/80 text-xs md:text-sm text-slate-200 flex flex-wrap items-center gap-x-5 gap-y-1.5">
                          <span className="text-emerald-400 font-bold flex items-center gap-1">
                            <User className="w-4 h-4" /> {g.buyer_name || g.line_name}
                          </span>
                          <span className="text-slate-200 font-mono flex items-center gap-1">
                            <Phone className="w-4 h-4 text-slate-400" /> {g.buyer_phone}
                          </span>
                          <span className="text-amber-400 font-medium flex items-center gap-1">
                            <MapPin className="w-4 h-4" /> {g.store_name}
                          </span>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2.5 self-end lg:self-auto flex-wrap justify-end">
                    ...
                    <div className="text-right pl-3 border-l border-slate-800">
                      <span className="text-xs text-slate-400 block">實收總額</span>
                  ...
                  <div className="space-y-2">
                    {g.items.map((it, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          className={`p-3.5 rounded-2xl border flex items-center justify-between text-xs md:text-sm ${
                            isFailed 
                        ...
                        >
                          <div className="flex items-center gap-2.5">
                            <span className={`font-medium ${isFailed ? 'line-through text-slate-400' : 'text-slate-100'}`}>
                              {it.name}
                            </span>
                            {it.selectedVariant && (
                              <span className="bg-slate-900 border border-slate-700 text-emerald-400 text-xs px-2 py-0.5 rounded">
                                規格: {it.selectedVariant}
                              </span>
                            )}
                            {isBought && (
                          ...
                          <div className={`font-mono text-sm ${isFailed ? 'line-through text-slate-500' : 'text-slate-300'}`}>
                            NT$ {it.price?.toLocaleString()} × {it.quantity} ={' '}
                            <span className="text-emerald-400 font-bold">
                              NT$ {(it.price * it.quantity).toLocaleString()}
                            </span>
                          </div>
                        </div>
                      )
